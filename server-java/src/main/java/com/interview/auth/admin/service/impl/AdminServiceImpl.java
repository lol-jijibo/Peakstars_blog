...
/**
 * 集中承接后台管理台的数据聚合、内容 CRUD 和实时心跳统计。
 * 后端统一负责模块差异映射、仪表盘快照生成和编辑日志记录，前端只消费标准化结构。
 */
@Service
@Slf4j
...
public class AdminServiceImpl implements AdminService {
    ...
    /**
     * 维护后台当前在线会话集合，给管理台实时在线人数提供数据来源。
     * 每个 clientId 的最后心跳时间只保存在内存中，足以满足单实例管理后台实时展示需求。
     */
    private final Map<String, LocalDateTime> onlineClientMap = new ConcurrentHashMap<>();

    /**
     * 维护后台仪表盘最近一段时间的趋势快照，供 ECharts 折线图直接使用。
     * 每次心跳或仪表盘刷新都会按节流规则写入快照，避免前端自己拼接趋势历史。
     */
    private final Deque<DashboardSnapshot> dashboardSnapshots = new ArrayDeque<>();

    /**
     * 记录后台心跳，驱动在线人数实时变化。
     * 同一个 clientId 反复心跳只刷新过期时间，不会重复累计在线人数。
     */
    @Override
    ...
    /**
     * 聚合后台仪表盘所需的全部数据。
     * 查询数据库当前总量后补齐趋势快照、模块分布和最近编辑，保证管理台首屏只打一条接口。
     */
    @Override
    ...
    /**
     * 按模块返回后台管理列表数据。
     * 不同表的原始结构会在这里映射成统一记录格式，便于前端共享一个表格和编辑抽屉。
     */
    @Override
    ...
    /**
     * 处理后台单条内容的新增和编辑保存。
     * 先按模块补齐默认值和业务主键，再执行幂等写入并记录最近编辑日志。
     */
    @Override
    @Transactional
    ...
    /**
     * 接收 Excel 批量导入后的多条记录写入请求。
     * 逐条复用单条写入逻辑，并在完成后补一条批量导入日志，便于后台回看导入动作。
     */
    @Override
    @Transactional
    ...
    /**
     * 软删除指定内容记录，让前台列表立即不再展示该内容。
     * 删除动作只更新 status，并同步记录一条编辑日志，便于管理台展示最近变更。
     */
    @Override
    @Transactional
    ...
    /**
     * 构建当前仪表盘汇总指标。
     * 把三类内容的浏览、评论和数量汇总后，再叠加在线人数和最近更新时间，形成顶部指标卡片。
     */
    ...
    /**
     * 构建模块维度统计，供 G2 做高级对比图表。
     * 每个模块统一输出内容量、浏览量和评论量，方便前端绘制多指标对比。
     */
    ...
    /**
     * 构建后台最近编辑时间线。
     * 直接读取最近日志并格式化时间，前端可以把结果作为动态列表展示。
     */
    ...
    /**
     * 构建后台热门内容列表，给管理台展示当前最值得关注的内容。
     * 把三类内容统一折算成浏览量后排序，便于管理员快速定位高热内容。
     */
    ...
    /**
     * 写入看天下期刊内容。
     * 把后台统一请求映射成期刊实体，并保留期刊封面和推荐值等展示字段。
     */
    ...
    /**
     * 把看天下实体转换成后台统一记录。
     * 沿用统一记录结构输出期刊封面和推荐值字段，方便后台在同一页面管理。
     */
     ...
     * 统一输出后台表格与编辑抽屉所需字段，前端不再依赖多个模块的不同对象结构。
     */
    private AdminContentRecordResponse toInterviewAdminRecord(Interview interview) {
        AdminContentRecordResponse response = new AdminContentRecordResponse();
        response.setType(TYPE_INTERVIEW);
        response.setId(String.valueOf(interview.getId()));
        ...
        response.setLikeCount(safeInt(interview.getLikes()));
        response.setCollectCount(safeInt(interview.getCollects()));
        response.setTrack(interview.getDifficulty());
        response.setTags(splitPipeValues(interview.getTagList()));
        return response;
    }

    /**
     * 写入面经内容。
     * 把后台统一请求映射成面经实体，并执行幂等保存。
     */
    private AdminContentRecordResponse saveInterview(String interviewKey, AdminContentUpsertRequest request) {
        ...
        interview.setPublishDate(parseDate(request.getPublishedAt()));
        interview.setDifficulty(defaultString(request.getTrack(), "easy"));
    ...
    /**
     * 解析面经主键，兼容新增（null）和编辑（已有 ID）两种场景。
     */
    private Long parseInterviewId(String key) {
        if (key == null || key.isBlank()) return null;
        try {
            return Long.parseLong(key.replaceAll("interview-", ""));
    ...
    /**
     * 写入后台编辑日志，支撑仪表盘最近编辑动态与今日编辑统计。
     * 每次保存、删除和批量导入都记录一条操作轨迹，便于后台管理员回看最近变化。
     */
    ...
    /**
     * 安全读取今日编辑次数，避免后台辅助日志表缺失时首页接口直接失败。
     * 优先查询 content_edit_log 统计值，若表不存在或 SQL 失败则自动降级为 0 并关闭后续日志查询。
     */
    ...
    /**
     * 安全读取最近编辑动态，避免日志表未建时阻断后台首页展示。
     * 日志查询失败后直接回退为空列表，让右侧动态面板可用空态正常渲染。
     */
    ...
    /**
     * 在后台日志表不可用时快速熔断辅助写入，保证核心内容管理接口继续可用。
     * 首次捕获数据库异常后记录警告并关闭日志能力，后续请求不再重复访问该表。
     */
    ...
    /**
     * 清理后台已过期的在线会话。
     * 超过过期窗口未继续上报心跳的 clientId 会被移除，保证在线人数接近实时状态。
     */
    ...
    /**
     * 规范后台内容类型，避免前后端传值不一致导致分支逻辑失效。
     * 统一转成小写后匹配固定模块 key，不支持的类型直接抛业务异常。
     */
    ...
    /**
     * 为新增内容生成稳定业务主键，同时允许编辑场景沿用原始主键。
     * 优先复用路径参数和请求体内主键，缺失时再按模块前缀和标题生成可读 key。
     */
    ...
    /**
     * 把后台输入的发布时间文案解析成数据库时间。
     * 兼容 yyyy-MM-dd 和 yyyy-MM-dd HH:mm 两种常见输入格式，减少表单录入限制。
     */
    ...
    /**
     * 统一格式化后台返回时间，方便前端直接显示。
     * 避免页面在多个表格和抽屉里重复处理日期格式。
     */
    ...
    /**
     * 把列表字段统一存成竖线拼接字符串，适配数据库持久化格式。
     * 忽略空值后再拼接，避免前端导入和手动编辑把空标签落进数据库。
     */
    ...
    /**
     * 把数据库中竖线分隔字段还原成数组，方便后台表单和标签组件直接使用。
     * 统一在后端拆分，前端无须关心持久化格式细节。
     */
    ...
    /**
     * 把可能为空的字符串字段统一补成默认值。
     * 保障 MySQL 非空列在新增和批量导入场景下始终拿到有效内容。
     */
    ...
    /**
     * 要求后台保存操作必须携带标题。
     * 标题是管理台表格、日志和主键生成的核心字段，缺失时直接阻断保存。
     */
    ...
    /**
     * 把数据库或请求里的可空整型统一转成非空数值。
     * 管理台统计和图表不接受 null，因此这里统一做兜底。
     */
    ...
    /**
     * 把数据库 0/1 标志位统一转成前端布尔值。
     * 避免前端对数值状态做额外判断，直接消费 true / false。
     */
    ...
    /**
     * 把推荐值空值统一转成 0，保证看天下期刊表单和图表输出稳定。
     * 防止空推荐值在后台表格和 Excel 导入导出场景下造成格式异常。
     */
    ...
    /**
     * 给后台新增作者自动补一个简短头像缩写。
     * 没有手填 initials 时，优先从名称前两个字符中生成，保证表格和头像卡片不空。
     */
    ...
    /**
     * 在内容尚未补齐正文时，为富文本字段生成最小可展示 HTML。
     * 确保 AiEditor 每次打开都有合法初始内容，避免空字符串导致编辑体验发散。
     */
