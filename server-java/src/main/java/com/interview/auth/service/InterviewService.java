...
/**
 * 业务目的：对外聚合面经模块的查询与操作服务，包括面经列表/详情/点赞/收藏和分类查询。
 * 业务逻辑：Service 层负责字段转换和数组拆分，让 Controller 只关注 HTTP 请求参收和统一响应封装。
 */
public interface InterviewService {

    /**
     * 业务目的：按分类、关键词和分页参数查询面经列表。
     * 业务逻辑：后端完成标签字符串拆分和日期格式化，返回统一的分页结构。
     *
    ...
    /**
     * 业务目的：为指定面经执行点赞操作。
     * 业务逻辑：使用原子 UPDATE 自增点赞数，返回最新的点赞数。
     *
     * @param id 面经 ID
    ...
    /**
     * 业务目的：为指定面经执行收藏操作。
     * 业务逻辑：使用原子 UPDATE 自增收藏数，返回最新的收藏数。
     *
     * @param id 面经 ID
    ...
    /**
     * 业务目的：获取面经分类列表，供前端导航和筛选使用。
     * 业务逻辑：按 sort_order 升序排列返回。
     *
     * @return 分类列表
     */
