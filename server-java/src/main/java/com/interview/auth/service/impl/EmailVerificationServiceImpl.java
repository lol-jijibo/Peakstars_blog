...
public class EmailVerificationServiceImpl implements EmailVerificationService {
    ...
    /**
     * 确保不在冷却时间内
     * 检查 Redis 中是否存在冷却标记，存在则说明60秒内已发送过，抛出异常
     *
     * @param email 邮箱地址
    ...
    /**
     * 确保未超过每日发送限制
     * 从 Redis 读取今日发送次数，如果达到或超过限制则抛出异常
     *
    ...
    /**
     * 增加每日发送计数
     * Redis 计数器自增，如果是当天第一次发送，设置过期时间到次日凌晨1点
     *
     * @param key Redis计数key
     */
     ...
     * 查询 Redis key 的 TTL，如果获取失败则返回默认冷却时间
     *
     * @param key Redis key
    ...
    /**
     * 清理注册验证码相关数据
     * 删除 Redis 中的验证码和尝试次数记录，用于验证失败或异常时的清理
     *
     * @param email 邮箱地址
     */
     ...
     * 拼接前缀、IP地址和日期，用于统计单个IP每天的发送次数，防止恶意攻击
     *
