...
/**
 * 业务目的：实现面经模块的全部业务逻辑，包括列表查询、详情获取、点赞/收藏和分类查询。
 * 业务逻辑：通过 MyBatis Mapper 获取原始数据后做字段映射和标签数组拆分，
...
public class InterviewServiceImpl implements InterviewService {
    ...
    /**
     * 业务目的：按分类、关键词和分页参数查询面经列表。
     * 业务逻辑：先查总数和列表，再将每行的 tagsStr 逗号字符串拆分成 List<String>，
    ...
    /**
     * 业务目的：根据面经 ID 获取完整详情。
     * 业务逻辑：如果面经不存在（查询结果为 null 或空）则抛 BusinessException。
    ...
    /**
     * 业务目的：为指定面经执行点赞操作。
     * 业务逻辑：使用原子 UPDATE 自增点赞数。如果影响行数为 0 表示面经不存在，抛异常。
     *         更新后重新查询返回最新点赞数。
     */
    @Override
    @Transactional
    ...
    /**
     * 业务目的：为指定面经执行收藏操作。
     * 业务逻辑：与 likeInterview 相同模式，使用原子 UPDATE 并返回最新收藏数。
     */
    @Override
    @Transactional
    ...
    /**
     * 业务目的：获取面经分类列表。
     * 业务逻辑：直接返回所有分类，按 sort_order 升序排列，无需额外过滤。
     */
    @Override
    ...
    /**
     * 业务目的：把面经列表的 Map 行数据转换成前端消费的 InterviewListResponse 对象。
     * 业务逻辑：标签从数据库的逗号分隔字符串拆分为 List<String>，
    ...
    /**
     * 业务目的：把面经详情的 Map 行数据转换成前端消费的 InterviewDetailResponse 对象。
     * 业务逻辑：在列表字段基础上增加 content 和 companyDesc，标签和日期处理与列表一致。
     */
    @SuppressWarnings("unchecked")
    ...
    /**
     * 业务目的：把 Category 实体转换成前端消费的结构。
     * 业务逻辑：直接透传 id / code / name 三个字段，与现有前端 getCategories 接口对齐。
     */
    ...
    /**
     * 业务目的：把数据库中的逗号分隔标签字符串拆分成 List&lt;String&gt;。
     * 业务逻辑：统一在 Service 层处理空字符串和 null 的边界，避免 Controller 和 View 层重复解析。
     */
