 ...
 * 已知业务错误按约定 code 返回，参数错误固定回 400，未知异常统一记录堆栈并输出可排查的错误类型。
 */
@Slf4j
...
public class GlobalExceptionHandler {
    ...
    /**
     * 把可预期的业务异常原样透传给前端，避免业务提示被系统错误覆盖。
     * 直接复用业务异常中的 code 和 message，前端可继续按原有协议展示提示。
     */
    ...
    /**
     * 统一处理请求体缺字段、字段格式不合法和 JSON 结构错误等输入异常。
     * 参数异常统一落成 400，同时记录异常类型，便于快速区分是校验失败还是请求结构错误。
     */
    @ExceptionHandler({
    ...
    /**
     * 兜底记录未识别系统异常，避免前端只收到无来源的泛化 500。
     * 完整打印异常堆栈到后端日志，前端仍保持统一的系统错误提示，避免内部实现细节外泄。
     */
