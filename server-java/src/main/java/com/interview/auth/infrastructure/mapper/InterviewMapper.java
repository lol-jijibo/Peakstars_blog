...
/**
 * 业务目的：承接面经模块的全部数据库查询，包括面经列表（支持分类/关键词/分页）、详情、点赞/收藏操作和分类列表。
 * 业务逻辑：面经列表通过 JOIN 一次查出公司名称和分类信息；标签使用 GROUP_CONCAT 聚合后在 Service 层拆分，避免 N+1 查询。
...
public interface InterviewMapper {
    ...
    /**
     * 业务目的：按分类、关键词和分页参数查询面经列表数据。
     * 业务逻辑：使用 LEFT JOIN 关联公司和分类表，标签用 GROUP_CONCAT 聚合为逗号分隔字符串。
    ...
    /**
     * 业务目的：查询面经列表的总记录数，用于前端分页器计算总页数。
     * 业务逻辑：过滤条件与 findInterviewList 保持一致，保证 count 与 list 的数据口径一致。
     *
     * @param category 分类编码
     * @param keyword  搜索关键词
     * @return 匹配条件的总记录数
    ...
    /**
     * 业务目的：查询一篇面经的完整详情，包含公司描述信息。
     * 业务逻辑：使用 LEFT JOIN 一次查出 interview、company、category 三表数据，
    ...
    /**
     * 业务目的：原子自增面经的浏览量，在详情页请求时异步触发。
     * 业务逻辑：与点赞/收藏相同使用原子 UPDATE，不阻塞主查询返回。
     *
     * @param id 面经 ID
    ...
    /**
     * 业务目的：原子自增面经的点赞数，避免并发写入丢失计数。
     * 业务逻辑：返回更新影响行数供 Controller 判断面经是否存在。
     *
     * @param id 面经 ID
     * @return 影响行数（0 表示面经不存在）
     */
    ...
    /**
     * 业务目的：查询点赞或收藏后的最新数值，用于前端实时刷新展示。
     * 业务逻辑：单独查询避免 UPDATE 返回值不携带字段的问题。
     *
     * @param id 面经 ID
    ...
    /**
     * 业务目的：查询所有已发布的面经分类列表，供前端分类导航和筛选下拉使用。
     * 业务逻辑：按 sort_order 升序排列，保证分类在前端展示的顺序可控。
     *
     * @return 分类列表
     */
