...
// 业务目的：提供 AI 热点页前端首版静态数据，后续可直接平移到 MySQL 表结构。
// 输入输出：页面读取热点数组作为列表输入，每项输出标题、摘要、作者、封面、技术标签和互动数据。
// 关键逻辑：同时保留推荐标记、发布时间和赛道字段，支撑前端先实现“推荐 / 最新”两种展示视图。
